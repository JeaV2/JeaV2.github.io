...
const fetchProjects = async () => await (await fetch('../data/projecten.json')).json();
...
async function createProjectCards() {
    ...
    try {
        ...
        data.forEach(project => {
            ...
            if (project.link) {
                cardHTML += `<a href="${project.link}" class="btn btn-primary" target="_blank" rel="noopener noreferrer">Bekijk Project</a>`;
            } else {console.warn(`No link for project: ${project.title}`);}

            if (project.github) {
                cardHTML += `<a href="${project.github}" class="btn btn-secondary" target="_blank" rel="noopener noreferrer">Bekijk op GitHub</a>`;
